...
const ARC = [
{ path: '/leadership', station: 'council', short: 'Day One', day: '02 Dec', kicker: 'Council' },
{ path: '/awards', station: 'awards', short: 'X. Awards', day: '02 Dec', kicker: 'Evening' },
{ path: '/keynotes', station: 'programme', short: 'Day Two', day: '03 Dec', kicker: 'Programme' },
{ path: '/workstream', station: 'workstream', short: 'Day Three', day: '04 Dec', kicker: 'Workstream' }];
...
function FDBHeader({ currentPath, onNav }) {
  const links = [
  { path: '/', label: 'Overview' },
  ...ARC.map((s) => ({ path: s.path, label: s.short }))];
  ...
  return (
    <header style={{
      ...
      <div className="fdb-header-inner" style={{
        ...
        <button onClick={() => onNav('/')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          <img src="assets/logos/fdb-banner.png" alt="Future Destination Brand." style={{ height: 36, width: 'auto', display: 'block' }} />
        ...
        <nav className="fdb-header-nav" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          {links.map((l) => {
            const active = currentPath === l.path || l.path === '/leadership' && currentPath === '/ai-act';
            return (
              <button key={l.path} className="fdb-nav-link" onClick={() => onNav(l.path)} style={{
...
function HostPartner({ label = 'Host partner', size = 24 }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
      ...
      <img src="assets/logos/failte-ireland.png" alt="Fáilte Ireland" style={{ height: size, width: 'auto', display: 'block' }} />
    </div>);
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '2.5rem 3rem' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <img src="assets/logos/fdb-banner.png" alt="Future Destination Brand." style={{ height: 26, width: 'auto', display: 'block' }} />
          ...
          <nav style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            <NavLink to="/leadership">Day One</NavLink>
            <NavLink to="/awards">X. Awards</NavLink>
            <NavLink to="/keynotes">Day Two</NavLink>
            <NavLink to="/workstream">Day Three</NavLink>
...
function DayHero({ station, eyebrow, title, lead, meta, children, motifSize = 420, motifTop = -20, motifRight = -80, motifSoft = false }) {
  return (
    <section className="fdb-rise" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="fdb-wrap" style={{ position: 'relative', padding: '3rem 3rem 3.5rem' }}>
        <HeroMotif state={station} size={motifSize} top={motifTop} right={motifRight} soft={motifSoft} />
...
Object.assign(window, {
  ...
  FDBHeader, FDBFooter, HostPartner,
